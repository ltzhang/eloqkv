...
foreach {pop} {BLPOP BLMPOP_LEFT} {
    ...
    test "PUSH resulting from BRPOPLPUSH affect WATCH" {
        ...
        # wait until the block command brpoplpush finished and ensure it finished before multi-exec
        wait_for_blocked_clients_count 0
    ...
    test {SWAPDB awakes blocked client} {
        ...
        $rd brpop k 5
        wait_for_blocked_clients_count 1
        # unsupported command: swap
        r swapdb 1 9
        $rd read
    } {k hello} {singledb:skip needs:support_cmd_later}
...
foreach {pop} {BLPOP BLMPOP_LEFT} {
    test "$pop when new key is moved into place" {
        ...
        # unsupported command: rename
        r rename bob{t} foo{t}
        set res [$rd read]
        $rd close
        set _ $res
    } {foo{t} hij} {needs:support_cmd_later}
    ...
    test "$pop when result key is created by SORT..STORE" {
        ...
        # TODO: remove the following lpop after implementing rename
        r lpop foo{t}
        r lpop foo{t}
        assert_equal {} [r lrange foo{t} 0 -1]
    ...
    foreach {pop} {BLPOP BRPOP BLMPOP_LEFT BLMPOP_RIGHT} {
        ...
        test "$pop: with non-integer timeout" {
            set rd [redis_deferring_client]
            r del blist1
            bpop_command $rd $pop blist1 1.1
            r rpush blist1 foo
            assert_equal {blist1 foo} [$rd read]
            assert_equal 0 [r exists blist1]
            $rd close
        ...
        test "$pop: with 0.001 timeout should not block indefinitely" {
            ...
            wait_for_blocked_clients_count 0 100 100
            assert_equal {} [$rd read]
            $rd close
        }
...
foreach type {listpack quicklist} {
    foreach {num} {250 500} {
        ...
        # eloqkv need not check encoding.
...
foreach {type large} [array get largevalue] {
    ...
    test "List encoding conversion when RDB loading" {
        ...
        r config set list-max-listpack-size $origin_conf
    } {OK} {needs:debug needs:redis_config}
    ...
    test "List of various encodings" {
        ...
        set _ $k
    } {12 0 9223372036854775808 2147483647 32767 127} {needs:redis_config}
