...
proc getInfoProperty {infostr property} {
    if {[regexp -lineanchor "^$property:(.*?)\r?\n" $infostr _ value]} {
        return $value
    }
}
...
proc status {r property} {
    set _ [getInfoProperty [{*}$r info clients] $property]
}
...
proc wait_for_blocked_clients_count {count {maxtries 100} {delay 50} {idx 0}} {
    wait_for_condition $maxtries $delay  {
        [s $idx blocked_clients] == $count
    } else {
        fail "Timeout waiting for blocked clients $count"
    }
